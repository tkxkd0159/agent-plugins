...
- **In AI-driven workflows:** unchanged in purpose; go/no-go should reference SLOs ([06](06-reliability-and-scale.md)) and eval results ([08](08-testing-and-quality.md)).

## Rollout / Migration Plan `Elevated`
...
- **In AI-driven workflows:** **elevated.** Progressive delivery is how you safely ship agent-generated code you cannot fully line-read — the rollout, not the code review, becomes the primary safety net (P3).

## Rollback Plan `Elevated`
...
- **In AI-driven workflows:** automate it and **test it** — fast automated rollback is the counterweight to fast, less-scrutinized AI delivery (P3).

## Release Notes `Core` `Executable`
...
- **In AI-driven workflows:** generate from merged PRs/commits, then human-edit for audience — low manual effort now (P1).

## Deprecation Plan `Conditional`
...
- **In AI-driven workflows:** without a deprecation plan, systems accumulate permanent technical debt agents must keep working around — record the sunset explicitly (P4).
