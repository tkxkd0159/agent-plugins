...
## Architecture Design Doc `Core` `Elevated`
...
- **In AI-driven workflows:** this is the stable context that constrains agent work — the invariants and boundaries an agent must not violate. Keep it durable and link it from the Agent Operating Manual ([07](07-implementation-and-execution.md)).

## Technical Design Doc `Core`
...
- **In AI-driven workflows:** for reversible work, prefer **spike-then-thin-design** — let an agent prototype, then capture what survived plus an ADR, rather than a long predictive design up front (P2). Heavy design is reserved for irreversible / high-blast-radius / cross-team work.

## Technical Specification `Merged`

> **Merged — synonym for Technical Design Doc in this system. Do not maintain both.** If your org uses "Technical Specification" to mean a combined requirements-plus-design document, that is fine; pick one name and one document. (Note: "TDD" here means Technical Design Doc, *not* Test-Driven Development.)

## RFC — Request for Comments `Conditional`
...
- **In AI-driven workflows:** the default "write an RFC to align humans" loses ground to "spike both approaches with agents, measure, then ADR." Keep RFCs for genuinely cross-team or breaking decisions (P2).

## ADR — Architecture Decision Record `Core` `Elevated`
...
- **In AI-driven workflows:** **one of the highest-leverage docs.** Agents have no memory and re-litigate every undocumented decision; a cheap ADR ends the re-debate permanently (P4). Supersede ADRs, never silently rewrite them.
