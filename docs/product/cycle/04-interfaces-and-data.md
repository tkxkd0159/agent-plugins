...
Every document in this phase is **executable by default**: the contract *is* the artifact (OpenAPI, proto, schema, migrations). Prose shrinks to intent plus a pointer to the machine-readable source of truth (P1).
...
- **Why it matters:** the machine-readable spec is the source of truth — agents generate clients, servers, mocks, and contract tests from it. A prose-only API doc is a liability (P1).
...
- **Why it matters:** schema-as-code (migrations) is the durable, agent-consumable artifact; agents read and generate against it (P1). Schema changes are high-blast-radius — favor heavy review (P2 exception).
...
- **Why it matters:** register schemas in a machine-readable registry so agents and compatibility checks can enforce evolution rules (P1).

## Integration Specification

- **Answers:** How do we depend on this other system, safely?
- **Use when:** integrating an external or internal system with a non-trivial contract, auth, or failure behavior.
- **Skip when:** simple dependencies — the API Spec + Dependency Map ([11](11-governance-and-learning.md)) cover them.
- **Owner:** Engineering.
...
- **Why it matters:** capture failure/fallback behavior explicitly — agents otherwise assume happy paths and ship brittle integrations (P3).
